...
import { ProjectOverviewView } from './manager/ProjectOverviewView';
...
import { ReportBuilderView } from './manager/ReportBuilderView';
import { ActionQueueView } from './manager/ActionQueueView';
import { FileText, BarChart3, UserCog, Briefcase, UserCheck, FolderKanban, ListTodo, CalendarDays, Activity, FileDown, ClipboardList } from 'lucide-react';

type SubTab = 'reports' | 'overview' | 'builder' | 'actions' | 'users' | 'projects' | 'employees' | 'departments' | 'tasks' | 'attendance' | 'activity';
export const ManagerDashboard: React.FC<{ backendData: BackendData; onRefreshData: () => void }> = ({ backendData, onRefreshData }) => {
  ...
  return <div className="space-y-5"><div className="flex flex-wrap items-center gap-2 bg-[#2C2A22]/50 p-1.5 rounded-xl border border-[#B89B5E]/30 shadow-inner">{tabs.map((t) => <button key={t.id} onClick={() => setSubTab(t.id)} className={`px-3 py-2 rounded-lg text-xs font-medium transition-all flex items-center gap-2 ${subTab === t.id ? 'bg-[#B89B5E] text-[#2C2A22] font-bold' : 'text-[#F2EEDD] hover:bg-white/10'}`}>{t.icon}<span>{t.label}</span></button>)}</div>
    {subTab === 'reports' && <ReportsView backendData={backendData} canLock />}{subTab === 'overview' && <ProjectOverviewView backendData={backendData} />}{subTab === 'builder' && <ReportBuilderView backendData={backendData} onRefreshData={onRefreshData} />}{subTab === 'actions' && <ActionQueueView backendData={backendData} onRefreshData={onRefreshData} />}{subTab === 'activity' && <ActivityView backendData={backendData} />}{subTab === 'attendance' && <AttendanceView backendData={backendData} onRefreshData={onRefreshData} />}{subTab === 'projects' && <ProjectsBuildingsView backendData={backendData} onRefreshData={onRefreshData} />}{subTab === 'tasks' && <TaskCategoriesView backendData={backendData} onRefreshData={onRefreshData} />}{subTab === 'employees' && <EmployeesView backendData={backendData} onRefreshData={onRefreshData} />}{subTab === 'departments' && <DepartmentsView backendData={backendData} onRefreshData={onRefreshData} />}{subTab === 'users' && <UsersView backendData={backendData} />}
  </div>;
};
