                      ...
                      return (
                        <div key={building.id} className="border border-[#DED2AC] rounded-xl p-4 space-y-3">
                          ...
                          <div>
                            ...
                            <div className="text-[10px] text-stone-500 mb-2">Enter each area size once. Area labels are generated automatically, and the building percentage is calculated from the total project area.</div>
                            <div className="flex flex-wrap gap-2">
                              {areas.map((area, index) => <span key={area.id} className="inline-flex items-center gap-1 px-2 py-1 bg-white border border-[#DED2AC] rounded-lg text-xs"><span className="text-stone-500">{/^\d+(\.\d+)?$/.test(area.name) ? `Area ${index + 1}` : area.name}</span><input type="number" min="0" step="0.01" defaultValue={area.area_m2 || 0} onBlur={(e) => run(() => updateAreaSize(area.id, Number(e.target.value) || 0), 'Area size saved.')} className="w-20 border border-[#DED2AC] rounded px-1 py-0.5 text-right" title="Square meters" /> <span className="text-stone-500">m²</span><button type="button" onClick={() => run(() => deleteArea(area.id), 'Area deleted.')} className="text-red-500 mr-1">×</button></span>)}
                              <form onSubmit={(e) => { e.preventDefault(); const areaM2 = Number(drafts[`${areaKey}-m2`] || 0); if (areaM2 < 0 || !drafts[`${areaKey}-m2`]) return; run(async () => { await addArea(building.id, `Area ${areas.length + 1}`, areaM2); setDraft(`${areaKey}-m2`, ''); }, 'Area added.'); }} className="inline-flex items-center gap-1">
                                <input type="number" min="0" step="0.01" value={drafts[`${areaKey}-m2`] || ''} onChange={(e) => setDraft(`${areaKey}-m2`, e.target.value)} placeholder="Area size (m²)" className="w-28 bg-white border border-[#DED2AC] rounded px-2 py-1 text-xs" />
