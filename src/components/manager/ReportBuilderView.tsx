...
import { CalendarRange, Download, FileDown, History, Loader2, Printer, Save, Search } from 'lucide-react';
...
function excelCell(value: unknown) { return String(value ?? '').replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;'); }
...
export const ReportBuilderView: React.FC<{ backendData: BackendData; onRefreshData: () => void }> = ({ backendData, onRefreshData }) => {
  ...
  const downloadExcel = () => {
    if (!preview) return;
    const summaryRows = Object.entries(preview.summary).map(([key, value]) => `<tr><th>${excelCell(key)}</th><td>${excelCell(value)}</td></tr>`).join('');
    const detailRows = (preview.details as any[]).map((row) => `<tr><td>${excelCell(row.project)}</td><td>${excelCell(row.building)}</td><td>${excelCell(row.task)}</td><td>${excelCell(row.department)}</td><td>${excelCell(row.employee)}</td><td>${excelCell(String(row.status).replaceAll('_', ' '))}</td><td>${excelCell(row.progress)}%</td><td>${excelCell(row.due || '')}</td></tr>`).join('');
    const html = `<html><head><meta charset="UTF-8"><style>table{border-collapse:collapse;font-family:Arial}th,td{border:1px solid #999;padding:8px}th{background:#3B4636;color:#fff}</style></head><body><h1>${excelCell(preview.title)}</h1><p>Generated: ${excelCell(new Date(preview.generated_at).toLocaleString())}</p><h2>Summary</h2><table>${summaryRows}</table><h2>Details</h2><table><thead><tr><th>Project</th><th>Building</th><th>Task</th><th>Department</th><th>Employee</th><th>Status</th><th>Progress</th><th>Due</th></tr></thead><tbody>${detailRows || '<tr><td colspan="8">No detail rows</td></tr>'}</tbody></table></body></html>`;
    const blob = new Blob([html], { type: 'application/vnd.ms-excel;charset=utf-8' }); const url = URL.createObjectURL(blob); const link = document.createElement('a'); link.href = url; link.download = `report_${preview.report_type}_${preview.period_start}_${preview.period_end}.xls`; document.body.appendChild(link); link.click(); link.remove(); URL.revokeObjectURL(url);
  };
  return <div className="space-y-5 print-report">
    <div className="bg-[#FBF8EF] border border-[#DED2AC] rounded-2xl p-5 no-print"><div className="flex items-start justify-between gap-4"><div><div className="flex items-center gap-2 text-[#3B4636] font-bold"><FileDown className="w-5 h-5 text-[#B89B5E]" />Report Builder</div><p className="text-xs text-stone-500 mt-1">Generate a reproducible report, save its data snapshot, then export it to Excel or print it.</p></div><div className="flex flex-wrap gap-2"><button type="button" onClick={downloadExcel} disabled={!preview} className="inline-flex items-center gap-2 bg-[#3B4636] text-white rounded-lg px-3 py-2 text-xs font-semibold disabled:opacity-40"><Download className="w-3.5 h-3.5" />Download Excel</button><button type="button" onClick={() => window.print()} disabled={!preview} className="inline-flex items-center gap-2 bg-white border border-[#DED2AC] text-[#3B4636] rounded-lg px-3 py-2 text-xs font-semibold disabled:opacity-40"><Printer className="w-3.5 h-3.5" />Print report</button></div></div>
