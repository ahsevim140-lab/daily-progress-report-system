...
export const AttendanceView: React.FC<{ backendData: BackendData; onRefreshData: () => void }> = ({ backendData, onRefreshData }) => {
  ...
  const reportRows = useMemo(() => datesBetween(rangeStart, rangeEnd).flatMap((reportDate) => employees.map((employee) => backendData.attendance.find((record) => record.employee_id === employee.id && record.attendance_date === reportDate) || ({
    employee_id: employee.id, employee_name: employee.name, department: employee.department, attendance_date: reportDate,
    entrance_time: '', status: 'present' as AttendanceStatus, hours_off: 0, note: '',
  }))), [backendData.attendance, employees, rangeEnd, rangeStart]);
  ...
  return (
    <div className="space-y-4 print:text-black">
      ...
      <div className="bg-[#FBF8EF] border border-[#DED2AC] rounded-2xl p-4 overflow-x-auto">
        ...
        <div className="text-[11px] text-stone-500 mb-3">{mode === 'entry' ? 'Choose a date to enter or update attendance.' : 'Choose an employee and date range, then print the filtered report.'}</div>
