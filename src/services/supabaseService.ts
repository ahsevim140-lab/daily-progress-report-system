...
export async function fetchBackendData(): Promise<BackendData> {
  const [deptRes, empRes, profileRes, loginRes, projRes, assignmentRes, buildRes, areaRes, taskRes, ptRes, activityRes, reportRes, attendanceRes] = await Promise.all([
  ...
  const firstError = [deptRes, empRes, profileRes, loginRes, projRes, assignmentRes, buildRes, areaRes, taskRes, ptRes, activityRes, reportRes, attendanceRes].find((r) => r.error)?.error;
  ...
  return {
    ...
    activities: (activityRes.data || []) as TaskActivity[], reportBatches: (reportRes.data || []).map((row: any) => ({ id: row.id, employee_id: row.employee_id ?? null, employee_name: row.employee_name, submitted_by: row.submitted_by ?? null, project_id: row.project_id, building_id: row.building_id, work_date: row.work_date, created_at: row.created_at, project_name: row.projects?.name, building_name: row.buildings?.name, lines: (row.report_lines || []) as ReportLine[] })) as ReportBatch[], attendance, loginAudits: (loginRes.data || []) as any[],
  };
}
